...
/**
 * Filter that enforces a simple sliding-window rate limit per client IP + URI.
 */
@Component
@Order(RATE_LIMITING)
public final class RateLimitingFilter extends OncePerRequestFilter {
    ...
    /** JSON payload returned in the response body when rate limit is exceeded. */
    private static final String RATE_LIMIT_JSON_PAYLOAD = "{\"error\":\"Rate limit exceeded\",\"message\":\"Too many requests.\"}";

    @Value("${app.rate-limit.requests-per-minute:60}")
    private int requestsPerMinute;

    /** Window (in minutes) after which rate limit entries are considered stale. */
    private static final int DEFAULT_CLEANUP_WINDOW_MINUTES = 2;

    private final Map<String, ClientRequestInfo> clientRequestCounts = new ConcurrentHashMap<>();
    ...
    protected void doFilterInternal(@NonNull final HttpServletRequest request,
        ...
        final String clientKey = clientIp + ':' + request.getRequestURI();
    ...
    private boolean isRateLimited(@NonNull final String clientKey) {
        ...
        return info != null && info.getRequestCount() > requestsPerMinute;
    }

    private String getClientIpAddress(@NonNull final HttpServletRequest request) {
    ...
    public void cleanupOldEntries() {
        ...
        clientRequestCounts.entrySet().removeIf(entry ->
                ChronoUnit.MINUTES.between(entry.getValue().getWindowStart(), now) > DEFAULT_CLEANUP_WINDOW_MINUTES);
    }
    ...
    private static final class ClientRequestInfo {
        ...
        ClientRequestInfo(final LocalDateTime windowStartParam, final int requestCountParam) {
